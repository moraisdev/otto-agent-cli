...
export function buildFusionLeadPlaybook(input: {
  ...
  return [
    ...
    `  otto sessions send ${companionKey} "<what you're doing + your question + which files/area to check>" -w --timeout 90`,
    `That blocks until ${peerName} replies (its analysis is the returned text). Pick \`--timeout\` to match the`,
    `question, not blindly: ~30s for a quick yes/no or sanity-check, ~90s for normal review (shown above), and`,
    `~180s when ${peerName} clearly needs to read multiple files or run tests/grep to answer. The CLI default`,
    `if you omit \`--timeout\` is 120s — fine for the typical mid-weight consult.`,
    `IMPORTANT: if the consult times out or errors, do NOT get stuck and do NOT retry it in a loop —`,
