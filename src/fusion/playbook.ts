...
export function buildFusionLeadPlaybook(input: {
  ...
  return [
    ...
    `  otto sessions send ${companionKey} "<what you're doing + your question + which files/area to check>" -w --timeout 45`,
    `That blocks until ${peerName} replies (its analysis is the returned text), and gives up after ~45s if it`,
    `stalls. IMPORTANT: if the consult times out or errors, do NOT get stuck and do NOT retry it in a loop —`,
    `note briefly that ${peerName} was unavailable this turn, then proceed SOLO and finish the user's request.`,
...
export function buildCompanionBrief(input: {
  ...
  return [
    ...
    ``,
    `Stay FOCUSED and time-boxed: the lead waits on your reply with a ~45s budget. Run a handful of`,
    `high-signal, targeted commands — NOT an exhaustive sweep or a long loop of shell calls. As soon as you`,
    `have enough to answer, STOP investigating and reply. A fast, specific answer beats a thorough-but-late one.`,
    ``,
