...
import { companionAgentId, LEGACY_COMPANION_AGENT_PREFIX } from "./companion-id.js";
...
export function ensurePeerCompanion(
  ...
  } else if (existing.provider !== peerProvider || existing.model !== model) {
    // The principal flipped (or model drift): re-point the companion to the peer.
    dbUpdateAgent(agentId, { provider: peerProvider, model });
