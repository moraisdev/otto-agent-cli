...
Otto uses a **Relation-Based Access Control** (REBAC) model. Every permission is a relation:
...
New agents are **closed by default** — they have no permissions until explicitly granted. Denied actions emit audit events to `otto.audit.denied`.

## Core Concepts

### Subjects

A subject is the entity requesting access. In Otto, subjects are always agents:
...
```bash
otto permissions grant agent:dev execute group:daemon       # daemon restart/status/logs
otto permissions grant agent:dev execute group:agents       # agents create/set/list
otto permissions grant agent:dev execute group:sessions     # sessions list/send/read
otto permissions grant agent:dev execute group:contacts     # contacts add/approve/block
otto permissions grant agent:dev execute group:routes       # routes add/remove/list
otto permissions grant agent:dev execute group:settings     # settings get/set
otto permissions grant agent:dev execute group:channels     # channels start/stop
otto permissions grant agent:dev execute group:heartbeat    # heartbeat set/enable
otto permissions grant agent:dev execute group:matrix       # matrix rooms/send
otto permissions grant agent:dev execute group:whatsapp.group  # WhatsApp group mgmt
otto permissions grant agent:dev execute group:service      # service install/start
```
...
```bash
otto permissions grant agent:dev execute group:agents_list    # Only agents list
otto permissions grant agent:dev execute group:daemon_restart # Only daemon restart
```
...
otto permissions grant agent:dev use tool:Edit
...
otto permissions grant agent:dev use toolgroup:execute
```
...
otto permissions grant agent:dev execute executable:*     # All executables
```
...
Some permissions are automatically derived from agent configuration fields and synced on daemon boot. These use `source=config` and are regenerated by `otto permissions sync`.
...
When a permission check fails, an audit event is published to the NATS topic `otto.audit.denied`:
...
```bash
nats sub "otto.audit.denied" --server nats://127.0.0.1:4223
```
...
# Examples
otto permissions revoke agent:dev execute group:contacts
otto permissions revoke agent:dev use toolgroup:write
```
...
otto permissions check agent:dev use tool:Read
...
```bash
otto permissions grant agent:main admin system:*
```
...
```bash
otto permissions grant agent:observer use toolgroup:read-only
otto permissions grant agent:observer execute group:agents_list
otto permissions grant agent:observer execute group:contacts
```
...
```bash
otto permissions init agent:vendas tool-groups
otto permissions init agent:vendas safe-executables
otto permissions grant agent:vendas execute group:contacts
otto permissions grant agent:vendas read_tagged_contacts system:leads
otto permissions grant agent:vendas access session:vendas-*
```
...
otto permissions grant agent:dev execute group:*        # CLI command groups
otto permissions grant agent:dev execute executable:*   # System binaries
```

**Wrong relation for executables:**
```bash
# WRONG — "use" is for SDK tools
otto permissions grant agent:dev use executable:git
